...
struct simd_sincos<double, Bits, Tag>
{
    ...
    force_inline
    static void eval_sincos_simpl(const simd_type& x, simd_type& ret_s, 
                    simd_type& ret_c)
    {   
        // it is assumed, that |x| is small, NaN, Inf values are not allowed
        ...
        //                      reconstruction
        //-----------------------------------------------------------------       
        simd_sincos_reconstruction<double, Bits,Tag>
            ::eval_both(qi, pc, ps, ret_s, ret_c);
    };
